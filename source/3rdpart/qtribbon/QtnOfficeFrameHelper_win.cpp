    ...
    DWORD event, HWND hWnd, LONG idObject, LONG /*idChild*/, DWORD /*dwEventThread*/, DWORD /*dwmsEventTime*/)
{
    ...
    {
        OfficeFrameHelperWin* frameHelperWin = (OfficeFrameHelperWin*)::SendMessage(hWnd, OfficeFrameHelperWin::m_msgGetFrameHook, 0, 0);
        if (frameHelperWin)
        {
...
/*!
    \class QtRibbon::OfficeFrameHelperWin
    \internal
*/
...
QString OfficeFrameHelperWin::getSystemMenuString(uint item) const
{
    ...
    if (hMenu)
    {
        ...
        if (len > 0)
        {
            ...
            ::GetMenuString(hMenu, id, (LPTSTR)lpBuf, len+1, MF_BYCOMMAND);
...
void OfficeFrameHelperWin::redrawFrame()
{
    if (!m_dwmEnabled)
    {
        ...
        if (opt.titleVisible)
        {
            ...
            if (!opt.active)
            {
                QtRibbon::StyleHintReturnThemeColor hintReturnInActive(tr("Window"), tr("CaptionTextInActive"));
                clrFrameCaptionText = static_cast<QRgb>(m_frame->style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintReturnInActive));
            }
            else
            {
                QtRibbon::StyleHintReturnThemeColor hintReturnActive(tr("Window"), tr("CaptionTextActive"));
...
void OfficeFrameHelperWin::drawTitleBar(QPainter* painter, const StyleOptionTitleBar& opt)
{
    ...
    //drawDwmCaptionText(painter, opt.rcTitleText, text[0] != Q_NULL ? QString::fromWCharArray(text) : opt.text, opt.state & QStyle::State_Active);  //TODO
    delete[] text;
...
void OfficeFrameHelperWin::drawDwmCaptionText(QPainter* painter, const QRect& rect, const QString& strWindowText, bool active)
{
    ...
    const UINT uFormat = /*DT_VCENTER | DT_LEFT |*/DT_CENTER | DT_END_ELLIPSIS | DT_SINGLELINE | DT_NOPREFIX;
    ...
    if (QTLWExtra* top = ((QtnHackWidget*)m_frame)->topData())
    {
        if (QWindow* window = (QWindow*)top->window)
        {
            if (QPlatformWindow* platformWindow = window->handle())
            {
				...
				int val = m_dwmEnabled ?  0 : (m_frameBorder/2);
...
/*
    QTLWExtra *top = ((QtnHackWidget *)m_frame)->d_func()->topData();
    int x1, y1, x2, y2;
    top->frameStrut.getCoords(&x1, &y1, &x2, &y2);
    top->frameStrut.setCoords(x1, 0, x2, y2);
*/
...
{
    if (m_hwndFrame && message->hwnd != m_hwndFrame && m_ribbonBar && message->hwnd == (HWND)m_ribbonBar->winId())
    {
    ...
    else if (message->message == WM_SIZE || message->message == WM_STYLECHANGED)
    {
        ...
        if ((m_szFrameRegion != szFrameRegion) || (message->message == WM_STYLECHANGED))
        {
            ...
            if (!m_dwmEnabled && (message->message == WM_STYLECHANGED))
            {
                ::SetWindowPos(m_hwndFrame, 0, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED);
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
        if (message->message == WM_SIZE && (message->wParam == SIZE_MAXIMIZED || message->wParam == SIZE_MINIMIZED || m_wasFullScreen))
        {
            m_wasFullScreen = false;
            return true;
        }
    ...
    if (message->message == OfficeFrameHelperWin::m_msgGetFrameHook)
    {
