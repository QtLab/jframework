...
{
    static QString className = QString("QtRibbon::RibbonBar");
    while (child) 
    {
...
void OfficeStyle::polish(QWidget* widget)
{
    ...
    if (widget->metaObject()->className() == QString("QComboBoxListView"))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::All, QPalette::HighlightedText, QColor(Qt::white));
        widget->setPalette(palette);
    }
    else if (widget->inherits("QtRibbon::QuickCustomizationPopup"))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::All, QPalette::Background, helper().getColor("Ribbon", "RibbonFace"));
        widget->setPalette(palette);
    }
    else if (widget->inherits("QTableView") || widget->inherits("QtRibbon::Grid"))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::All, QPalette::Background, helper().getColor("Ribbon", "RibbonFace"));
        widget->setPalette(palette);
    }
    else if (qobject_cast<QTableView*>(widget) || widget->inherits("QtRibbon::Grid"))
    {
        QPalette palette = widget->palette();
...
QString OfficeStyle::getStyle(const QString& str) const
{
    ...
    {
        case Office2007Blue  :
                strStyle += ":/OfficeStyle/Office2007Blue/";
            break;
        case Office2007Black :
                strStyle += ":/OfficeStyle/Office2007Black/";
            break;
        case Office2007Silver :
                strStyle += ":/OfficeStyle/Office2007Silver/";
            break;
        case Office2007Aqua :
                strStyle += ":/OfficeStyle/Office2007Aqua/";
            break;
        case Windows7Scenic :
                strStyle += ":/OfficeStyle/Windows7Scenic/";
            break;
        case Office2010Silver :
                strStyle += ":/OfficeStyle/Office2010Silver/";
            break;
        case Office2010Blue :
                strStyle += ":/OfficeStyle/Office2010Blue/";
            break;
        case Office2010Black :
                strStyle += ":/OfficeStyle/Office2010Black/";
            break;
        default:
            Q_ASSERT(false);
            break;
...
QRect OfficeStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex* opt, SubControl sc, const QWidget* widget) const
{
    ...
    {
        case CC_TitleBar:
            if (widget && (widget->metaObject()->className() == QString("QtRibbon::OfficePopupWindow")) && d.m_popupProxy)
...
int OfficeStyle::pixelMetric(PixelMetric pm, const QStyleOption* opt, const QWidget* w) const
{
    ...
    switch(pm)
    {
        case PM_TitleBarHeight:
            if (w && (w->metaObject()->className() == QString("QtRibbon::OfficePopupWindow")) && d.m_popupProxy)
...
bool OfficeStyle::drawTabBarTabLabel(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    if (const QStyleOptionTabV3* tab = qstyleoption_cast<const QStyleOptionTabV3*>(opt))
    {
        ...
        color = helper().getColor(tr("TabManager"), tr("TextColor"));
...
/*!
    \class QtRibbon::OfficeStylePlugin
    \internal
*/
