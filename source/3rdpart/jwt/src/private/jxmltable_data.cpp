...
bool JNumericValue::inRange(qreal &value, bool increase) const
{
    if ((d->range->minimumEdge()
         ? JWT::fGreaterOrEqual(value, d->range->minimum())
         : JWT::fGreater(value, d->range->minimum()))
            && (d->range->maximumEdge()
                ? JWT::fLessOrEqual(value, d->range->maximum())
                : JWT::fLess(value, d->range->maximum()))) {
        return true;
    } else {
        if (increase) {
...
bool JNumericValue::without(qreal &value, bool increase)
{
    ...
    while (iter.hasNext()) {
        ...
        if ((without->minimumEdge()
             ? JWT::fGreaterOrEqual(value, without->minimum())
             : JWT::fGreater(value, without->minimum()))
                && (without->maximumEdge()
                    ? JWT::fLessOrEqual(value, without->maximum())
                    : JWT::fLess(value, without->maximum()))) {
            value = increase
