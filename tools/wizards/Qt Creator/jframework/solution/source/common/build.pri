...
contains(DEFINES, JFRAME_FACADE_LIB):!contains(DEFINES, JFRAME_FACADE_BUILD) {
    ...
    win32:CONFIG(release, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_facade
    else:win32:CONFIG(debug, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_facaded
    else:unix:LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_facade
}

contains(DEFINES, JFRAME_FACTORY_LIB):!contains(DEFINES, JFRAME_FACTORY_BUILD) {
    ## import jframe_factory library
    win32:CONFIG(release, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_factory
    else:win32:CONFIG(debug, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_factoryd
    else:unix:LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_factory
    INCLUDEPATH += $$(jframe_dir)/include/jframe/factory
    DEPENDPATH += $$(jframe_dir)/include/jframe/factory
}

contains(DEFINES, JFRAME_KERNEL_LIB):!contains(DEFINES, JFRAME_KERNEL_BUILD) {
    ## import jframe_layout library
    win32:CONFIG(release, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_kernel
    else:win32:CONFIG(debug, debug|release):LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_kerneld
    else:unix:LIBS += -L$$(jframe_dir)/lib/jframe -ljframe_kernel
    INCLUDEPATH += $$(jframe_dir)/include/jframe/kernel
    DEPENDPATH += $$(jframe_dir)/include/jframe/kernel
}
