...
{
    // 加载库
    std::string filePath = std::string(appDirPath()).append("/").append(libraryPrefix())
    ...
    // 加载 jfwconfiggui 模块
    filePath = std::string(appDirPath()).append("/").append(libraryPrefix())
            .append("jfwconfiggui").append(librarySuffix(false));

    // 获取 ExecGui 导出接口
    typedef int (J_ATTR_CDECL*ExecGui)(int, char**);
    ExecGui execGui = (ExecGui)
            JLibrary::resolve(filePath, "ExecGui");
    if (!execGui) {
        return -1;      // 获取导出接口失败
    }

    //
    return execGui(argc, argv);
}
